...
df = pd.read_excel("spread - BAN.xlsx", header=None)
headers = df.iloc[0]
df = df.iloc[1:]
df.columns = headers
...
df = df.drop(columns=["trafione", *BAN_LIST], axis=1, errors='ignore')

theme_picks = []
for name, themes in df.items():
    filtered_themes = themes[~themes.str.startswith("#")]
    themes_counts = filtered_themes.value_counts()
...
def select_winner(weights, df):
    ...
    winner_themes = df[winner][~themes.str.startswith("#")].dropna().tolist()
